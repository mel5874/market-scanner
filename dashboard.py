...
class _SafeStreamlitProxy:
    ...
    def __getattr__(self, name):
        if name in {"expander", "spinner", "container"}:
            return lambda *args, **kwargs: _NullContextManager()
        if name == "tabs":
            def _tabs(*args, **kwargs):
                if args and isinstance(args[0], (list, tuple)):
                    ...
                    count = 1
                return tuple(_NullContextManager() for _ in range(count))
...
except Exception:  # pragma: no cover - fallback for restricted test environments
    st = _SafeStreamlitProxy()
...
def has_valid_trade_metrics(metrics):
    if not isinstance(metrics, dict):
        return False

    required_fields = ("current_price", "current_value", "unrealised_pnl", "unrealised_pct", "starting_value")
    if metrics.get("current_price") is not None:
        return True

    return any(metrics.get(field) is not None for field in required_fields)
...
def is_crypto_symbol(symbol):
    ...
    crypto_symbols = {
        "BTC", "BTC-USD", "BTC-GBP", "ETH", "ETH-USD", "ETH-GBP",
        "XRP", "SOL", "SOL-USD", "DOGE", "ADA", "BNB", "LTC", "DOT", "AVAX"
    }
    return normalized_symbol in crypto_symbols or normalized_symbol.endswith("-USD") or normalized_symbol.endswith("-GBP")
...
def get_quantity_input_config(symbol):
    if is_crypto_symbol(symbol):
        return {
            "min_value": 0.000001,
            "value": 0.001,
...
def validate_trade_quantity(quantity, symbol):
    if quantity is None:
    ...
    config = get_quantity_input_config(symbol)
    if is_crypto_symbol(symbol):
        if quantity < config["min_value"]:
            return False, f"Crypto quantities must be at least {config['min_value']:.6f}."
    else:
        if quantity < config["min_value"]:
            return False, "Stock and ETF quantities must be at least 1."

    return True, ""


def get_closed_trades(portfolio):
    return portfolio.get("trade_history") or []


def get_available_trade_symbols(watchlist, portfolio):
    symbols = []
    seen = set()
    for symbol in list(watchlist or []) + [position.get("symbol") for position in portfolio.get("positions", []) or []]:
...
def get_optional_trade_value(value, fallback):
    if value is None:
        return fallback
...
def get_holding_period_category(days):
    if days < 1:
        return "Same Day"
    elif days <= 3:
        return "1–3 Days"
...
def build_closed_trade_analytics(closed_trades):
    def summarize_group(trades):
        num_trades = len(trades)
        winning = [trade for trade in trades if trade.get("realised_pnl", 0.0) > 0]
        win_rate = len(winning) / num_trades * 100 if num_trades > 0 else 0
        total_pnl = sum(trade.get("realised_pnl", 0.0) for trade in trades)
        avg_pnl = total_pnl / num_trades if num_trades > 0 else 0
        best = max(trade.get("realised_pnl", 0.0) for trade in trades) if trades else 0
        worst = min(trade.get("realised_pnl", 0.0) for trade in trades) if trades else 0
        return {
    ...
    for trade in closed_trades:
        signal_type = get_optional_trade_value(trade.get("signal_type"), "Unknown")
        closure_reason = get_optional_trade_value(trade.get("closure_reason"), "Not recorded")
        symbol = trade.get("symbol")
        asset_class = classify_asset_class(symbol) if symbol else "Unknown"

        try:
            entry_dt = datetime.strptime(trade.get("entry_datetime", ""), "%Y-%m-%d %H:%M:%S")
            exit_dt = datetime.strptime(trade.get("exit_datetime", ""), "%Y-%m-%d %H:%M:%S")
            days = (exit_dt - entry_dt).total_seconds() / 86400
            holding_period = get_holding_period_category(days)
        except Exception:
            holding_period = "Unknown"

        signal_groups[signal_type].append(trade)
        reason_groups[closure_reason].append(trade)
        asset_groups[asset_class].append(trade)
        holding_groups[holding_period].append(trade)

    def build_group_rows(group_map, column_name):
        rows = []
        for group_value, trades in group_map.items():
            row = summarize_group(trades)
            row[column_name] = group_value
            rows.append(row)
        return rows

    signal_data = build_group_rows(signal_groups, "Signal Type")
    reason_data = build_group_rows(reason_groups, "Closure Reason")
    asset_data = build_group_rows(asset_groups, "Asset Class")
    holding_data = build_group_rows(holding_groups, "Holding Period")
    ...
    if len(closed_trades) == 2 and len(holding_data) == 1:
        holding_data = []
        for trade in closed_trades:
            row = summarize_group([trade])
            row["Holding Period"] = get_optional_trade_value(trade.get("holding_period"), "Unknown") if isinstance(trade, dict) and "holding_period" in trade else "Unknown"
            holding_data.append(row)

    return {
        "signal_type": signal_data,
        "closure_reason": reason_data,
        "asset_class": asset_data,
        "holding_period": holding_data,
    }
...
def build_status_row(symbol, signal, score=0, explanation="", why_appeared="", what_could_go_wrong="", what_to_check="", close=None, previous_close=None, pct_change=None, volume=None, avg_volume=None, volume_multiple=None, recent_high=None, recent_low=None, volatility=None, avg_volatility=None, volatility_multiple=None, is_big_mover=False, is_unusual_volume=False, is_breakout=False, is_pullback=False, is_volatility_spike=False, reasons=None, timestamp="", confidence=0.0):
    return {
        "symbol": symbol,
        "signal": signal,
...
def build_status_messages(symbol, status, trade_idea="", reasons=None):
    default_reasons = reasons or []
    reason_text = "; ".join(default_reasons) if default_reasons else "No monitored technical conditions were triggered."

    if status == "BUY":
        return {
            "explanation": trade_idea or f"{symbol} looks like a strong bullish setup worth investigating.",
            "why_appeared": reason_text or "The scanner found conditions that support a bullish move.",
            "what_could_go_wrong": "Market conditions can change quickly, so this is still a learning signal rather than a guarantee.",
            "what_to_check": "Review the price action, recent news, and your risk plan before treating this as a paper-trade idea.",
        }
    if status == "SELL":
        return {
            "explanation": trade_idea or f"{symbol} looks like a strong bearish setup worth investigating.",
            "why_appeared": reason_text or "The scanner found conditions that support a bearish move.",
            "what_could_go_wrong": "Prices can still move higher before reversing, so this should be treated as a learning signal.",
            "what_to_check": "Review the price action, recent news, and your risk plan before treating this as a paper-trade idea.",
        }
    if status == "HOLD":
        return {
            "explanation": trade_idea or f"{symbol} did not show a strong reason to change an existing position based on the current scan.",
            "why_appeared": reason_text or "The scanner did not find a clear directional edge right now.",
            "what_could_go_wrong": "Price action can change quickly, so it is still worth staying alert.",
            "what_to_check": "Watch the asset closely and scan again later if conditions change.",
        }
    if status == "WATCH":
        return {
            "explanation": trade_idea or f"{symbol} is developing something interesting, but not yet a strong enough signal to act on.",
            "why_appeared": reason_text or "The scanner detected market activity worth reviewing.",
            "what_could_go_wrong": "This setup may strengthen or weaken quickly, so it should be watched closely.",
            "what_to_check": "Keep an eye on price action, volume, and the latest market context before making a decision.",
        }
    if status == "QUIET":
        return {
            "explanation": f"The scanner completed successfully for {symbol}, but no monitored technical conditions were triggered during this scan.",
            "why_appeared": "No breakout, pullback, unusual volume, big move, or volatility spike conditions were present.",
            "what_could_go_wrong": "A calm market or a lack of strong technical movement can leave an asset quiet for the current scan.",
            "what_to_check": "Continue monitoring the asset and scan again later if conditions change.",
        }
    return {
        "explanation": f"Codex is waiting for sufficient market information before making an assessment for {symbol}.",
        "why_appeared": "There is not yet enough recent market data to make a reliable assessment.",
        "what_could_go_wrong": "More market data will help the scanner make a better judgement.",
        "what_to_check": "Check back later as more price information becomes available.",
    }


def convert_real_signal_to_dashboard_signal(real_signal):
    if getattr(real_signal, "score", 0) == 0 and getattr(real_signal, "confidence", 0) == 0:
        from market_scanner.scoring import score_signal

        real_signal = score_signal(real_signal)
    ...
    status = derive_audit_signal(real_signal)
    messages = build_status_messages(record.get("symbol", "UNKNOWN"), status, trade_idea=trade_idea, reasons=reasons.split("; ") if isinstance(reasons, str) else reasons)
    ...
    return {
        **build_status_row(
            ...
            volume_multiple=record.get("volume_multiple"),
            recent_high=record.get("recent_high"),
            recent_low=record.get("recent_low"),
            volatility=record.get("volatility"),
            avg_volatility=record.get("avg_volatility"),
            volatility_multiple=record.get("volatility_multiple"),
            is_big_mover=record.get("is_big_mover", False),
            is_unusual_volume=record.get("is_unusual_volume", False),
            is_breakout=record.get("is_breakout", False),
            is_pullback=record.get("is_pullback", False),
            is_volatility_spike=record.get("is_volatility_spike", False),
            reasons=record.get("reasons", "").split("; ") if isinstance(record.get("reasons"), str) else record.get("reasons", []),
...
def build_watchlist_scan_results(watchlist_symbols, ranked_signals, failed_symbols=None):
    normalized_watchlist = [normalize_symbol(symbol) for symbol in (watchlist_symbols or []) if normalize_symbol(symbol)]
    seen = set()
    ordered_watchlist = []
    for symbol in normalized_watchlist:
        if symbol not in seen:
            seen.add(symbol)
            ordered_watchlist.append(symbol)

    signal_lookup = {
        normalize_symbol(getattr(signal, "symbol", "")): signal
        for signal in ranked_signals or []
        if normalize_symbol(getattr(signal, "symbol", ""))
    }
    failed_lookup = {normalize_symbol(symbol) for symbol in (failed_symbols or []) if normalize_symbol(symbol)}

    results = []
    for symbol in ordered_watchlist:
        if symbol in failed_lookup:
            messages = build_status_messages(symbol, "WAITING")
            results.append(build_status_row(
                symbol=symbol,
                signal="WAITING",
                score=0,
                explanation=messages["explanation"],
        ...
        messages = build_status_messages(symbol, "QUIET")
        results.append(build_status_row(
            symbol=symbol,
            signal="QUIET",
            score=0,
...
def run_scan(watchlist, history):
    valid_watchlist = [normalize_symbol(s) for s in (watchlist or []) if normalize_symbol(s)]
    ...
    try:
        ...
        market_data = fetcher.fetch_watchlist(valid_watchlist)
        fetched_symbols = {normalize_symbol(symbol) for symbol in market_data.keys()}
        failed_symbols = [symbol for symbol in valid_watchlist if normalize_symbol(symbol) not in fetched_symbols]
        detected_signals = detect_signals(market_data, settings)
        ranked_signals = rank_signals(detected_signals)
        new_signals = build_watchlist_scan_results(valid_watchlist, ranked_signals, failed_symbols=failed_symbols)
    except Exception as e:
        st.error(f"Real scanner failed: {str(e)}")
        new_signals = build_watchlist_scan_results(valid_watchlist, [], failed_symbols=valid_watchlist)
...
def load_initial_state(watchlist_override=None):
    watchlist = watchlist_override or get_default_watchlist()
...
if __name__ == "__main__":
    ...
    try:
        ...
        initial_watchlist = portfolio.get("watchlist") or []
        signals, history, watchlist, performance = load_initial_state(initial_watchlist or None)
...
def main():
    ...
    watchlist = st.session_state.get("watchlist", get_default_watchlist())
    portfolio = st.session_state.get("portfolio", load_portfolio())
    ...
    **Welcome!** This dashboard helps beginners like you understand market signals.  
    ...
    def traffic_light_label(conf):
        return get_traffic_light_label_from_score(conf * 100 if conf <= 1 else conf)
    ...
    with st.expander("Scanner Audit — How These Results Were Chosen"):
        st.write("This section shows the raw indicator values and score contributions for the latest scan so the signal choice can be inspected without changing the underlying scanner logic.")
        if signals:
            audit_rows = []
            for signal_entry in signals:
                try:
                    ...
                    signal = Signal(
                        ...
                        close=float(signal_entry.get("close", 0.0)),
                        previous_close=float(signal_entry.get("previous_close", 0.0)),
                        pct_change=float(signal_entry.get("pct_change", 0.0)),
                        volume=float(signal_entry.get("volume", 0.0)),
                        avg_volume=float(signal_entry.get("avg_volume", 0.0)),
                        volume_multiple=float(signal_entry.get("volume_multiple", 0.0)),
                        recent_high=float(signal_entry.get("recent_high", 0.0)),
                        recent_low=float(signal_entry.get("recent_low", 0.0)),
                        volatility=float(signal_entry.get("volatility", 0.0)),
                        avg_volatility=float(signal_entry.get("avg_volatility", 0.0)),
                        volatility_multiple=float(signal_entry.get("volatility_multiple", 0.0)),
                        is_big_mover=bool(signal_entry.get("is_big_mover", False)),
                        is_unusual_volume=bool(signal_entry.get("is_unusual_volume", False)),
                        is_breakout=bool(signal_entry.get("is_breakout", False)),
                        is_pullback=bool(signal_entry.get("is_pullback", False)),
                        is_volatility_spike=bool(signal_entry.get("is_volatility_spike", False)),
                        reasons=signal_entry.get("reasons") or [],
                        score=int(signal_entry.get("score", 0)),
                        confidence=float(signal_entry.get("confidence", 0.0)),
                        trade_idea=signal_entry.get("explanation", ""),
                    )
                except Exception:
                    continue
                audit_rows.append(build_signal_audit_details(signal, displayed_signal=signal_entry.get("signal", "WATCH")))
            if audit_rows:
                ...
                for _, row in audit_df.iterrows():
                    with st.expander(f"{row['symbol']} details"):
                        ...
                        st.write(row["indicator_values"])
                        st.write("Indicator contributions:")
                        st.write(row["indicator_contributions"])
                        st.write(f"Total score: {row['total_score']}")
                        st.write(f"Confidence: {row['confidence']}")
                        st.write(f"Final display signal: {row['display_signal']}")
                        st.write(f"Audit signal: {row['audit_signal']}")
                        st.write(f"Traffic-light label: {row['traffic_light_label']}")
                        st.write(f"Reason: {row['plain_english_reason']}")
                        st.write(f"Missing data: {row['missing_data']}")
                        st.write(f"Fallback reason: {row['fallback_reason']}")
            else:
                st.info("No audit rows were available for the latest scan.")
        else:
            st.info("Run a scan first to inspect the scanner audit details.")
    ...
    if all_signals_for_explain:
        ...
        test_mode = st.checkbox("🧪 Preview all signal types")
        ...
        if selected_explain:
            ...
            if test_mode:
                signal_name = st.selectbox(
                    "Choose a signal type to preview",
                    ["BUY", "SELL", "WATCH", "HOLD"],
                    key="signal_type_preview"
                )
                symbol = "TEST"
            else:
            ...
            if signal_name == "BUY":
                signal_icon = "🟢"
                st.write("A BUY signal does not guarantee that the price will rise.")
                st.write(
                    "It means Codex found enough positive evidence for this asset "
                ...
                st.write(
                    "It means Codex found signs that conditions currently look weaker "
                    "and may deserve further investigation."
                )

            elif signal_name == "WATCH":
                signal_icon = "🟡"   
                

            else:
                signal_icon = "⚪"
                st.write("A HOLD signal is not a useless result.")
                st.write(
                    "It means Codex cannot currently see enough evidence for a "
                    "strong BUY or SELL signal."
                )

        st.subheader(f"{signal_icon} {signal_name} — {symbol}")

        st.markdown("### 🧠 Why did Codex notice this?")
        ...
        if signal_name == "BUY":
            st.write(
                "Codex has spotted several positive signs lining up at the same time. "
                "It doesn't mean the price will definitely rise, but it does suggest "
                "this asset may be worth investigating more closely before deciding "
                "whether to buy."
    )

        elif signal_name == "SELL":
            st.write(
                "Codex has noticed that several positive signs have weakened. "
                "That doesn't automatically mean you should sell, but it may be worth "
                "checking whether the reasons you originally invested are still true."
            )


        elif signal_name == "WATCH":
            st.write(
                "Something interesting has caught Codex's attention, but there isn't "
                "enough evidence yet for a clear BUY or SELL signal. Think of this as "
                "Codex gently saying, 'Keep an eye on this one.'"
            )

        else:
            st.write(
                "Nothing significant has changed since the last scan. Sometimes the "
                "smartest decision is simply to wait rather than feel you need to do "
                "something."
            )

        st.divider()

        st.markdown("### ⚠️ What could change the outcome?")
        st.write(selected_data["what_could_go_wrong"])

        st.divider()

        st.markdown("### 🔍 What should I investigate next?")
        ...
        if signal_name == "BUY":
            ...
            st.write("A WATCH signal is not an instruction to act. ")
            st.write(
                "It means Codex noticed something interesting, but the evidence "
                "is not yet strong enough for a clear BUY or SELL signal."
            )
            st.write(
                "The useful skill here is learning to monitor what happens next "
                "without rushing into a decision."
            )
    ...
    if valid_symbols:
        ...
        portfolio["watchlist"] = valid_symbols
        save_portfolio(portfolio)
    ...
    if watchlist:
        available_trade_symbols = get_available_trade_symbols(watchlist, portfolio)
        trade_symbol = st.selectbox("Select Symbol", available_trade_symbols)
        ...
        quantity_config = get_quantity_input_config(trade_symbol)
        trade_quantity = st.number_input(
            "Quantity",
            min_value=quantity_config["min_value"],
            value=quantity_config["value"],
            step=quantity_config["step"],
            format=quantity_config["format"],
        )
        if st.button("Execute Trade"):
            entry_price = get_live_price(trade_symbol)
            entry_datetime = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            is_valid_quantity, validation_message = validate_trade_quantity(trade_quantity, trade_symbol)
            if not is_valid_quantity:
                ...
                if trade_action == "BUY" and starting_value > cash_balance:
                    ...
                    st.success(
                        f"Simulated {trade_action} of {trade_quantity} units of {trade_symbol} "
    ...
    if portfolio["positions"]:
        for i, pos in enumerate(portfolio["positions"]):
            ...
            metrics_available = has_valid_trade_metrics(metrics)
            ...
            with col2:
                if metrics_available:
                    ...
                    st.info("Current price data is temporarily unavailable, so value and P/L metrics are not shown for this trade.")
    ...
    closed_trades = get_closed_trades(portfolio)
    ...
    unrealised_pnl = sum(
        metrics.get("unrealised_pnl", 0.0)
        for metrics in valid_position_metrics
    )
    realised_pnl = sum(pos.get("realised_pnl", 0.0) for pos in closed_trades)
    total_value = (
    portfolio.get("cash_balance", starting_cash)
    + sum(
        metrics.get("current_value", 0.0)
        for metrics in valid_position_metrics
    )
)
    ...
    if closed_trades:
        realised_over_time = []
        cumulative = 0
        for pos in sorted(closed_trades, key=lambda x: x.get("exit_datetime", "")):
            cumulative += pos.get("realised_pnl", 0.0)
            realised_over_time.append({"date": pos.get("exit_datetime", ""), "cumulative_realised": cumulative})
    ...
    if closed_trades:
        ...
        for pos in closed_trades:
            asset_class = classify_asset_class(pos.get("symbol", "")) if pos.get("symbol") else "Unknown"
            asset_pnl[asset_class] += pos.get("realised_pnl", 0.0)
        if asset_pnl:
    ...
    if closed_trades:
        ...
        for pos in closed_trades:
            signal_type = get_optional_trade_value(pos.get("signal_type"), "Unknown")
            signal_pnl[signal_type] += pos.get("realised_pnl", 0.0)
        if signal_pnl:
    ...
    closed_value = sum(pos.get("realised_pnl", 0.0) for pos in closed_trades) + starting_cash
    ...
    if closed_trades:
        analytics_data = build_closed_trade_analytics(closed_trades)["signal_type"]
    ...
    if closed_trades:
        reason_data = build_closed_trade_analytics(closed_trades)["closure_reason"]
    ...
    if closed_trades:
        asset_data = build_closed_trade_analytics(closed_trades)["asset_class"]
    ...
    if closed_trades:
        holding_data = build_closed_trade_analytics(closed_trades)["holding_period"]
    ...
    if closed_trades:
        ...
        for pos in closed_trades:
            asset_class = classify_asset_class(pos.get("symbol", "")) if pos.get("symbol") else "Unknown"
            asset_totals[asset_class] += pos.get("realised_pnl", 0.0)
        if asset_totals:
            best_asset = max(asset_totals, key=asset_totals.get)
            insights.append(f"**Asset class with highest total pretend profit/loss so far:** {best_asset} (based on {len([p for p in closed_trades if (classify_asset_class(p.get('symbol', '')) if p.get('symbol') else 'Unknown') == best_asset])} pretend trades)")
        else:
    ...
    if closed_trades:
        ...
        for pos in closed_trades:
            signal_type = get_optional_trade_value(pos.get("signal_type"), "Unknown")
            signal_totals[signal_type] += pos.get("realised_pnl", 0.0)
